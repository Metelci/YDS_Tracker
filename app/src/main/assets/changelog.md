...
### Settings - Dynamic About Dialog

- **Automatic Release Notes**: About dialog now reads from CHANGELOG.md automatically
  - No more hardcoded release notes in code
  - Single source of truth for version information
  - Zero maintenance for future releases

- **Markdown Parsing**: Custom lightweight parser supports formatting
  - `**Bold text**` rendered with FontWeight.Bold
  - `` `Code snippets` `` shown in monospace font
  - `- Bullet lists` with proper indentation
  - `### Section headers` displayed as bold titles

- **Smart Error Handling**: Three-level fallback system
  - Primary: Load and parse from assets/changelog.md
  - Fallback 1: If file missing, use hardcoded v2.9.75 notes
  - Fallback 2: If parse fails, graceful degradation
  - Logs warnings for debugging (ChangelogLoader, ChangelogParser tags)

- **Version Validation**: Automatic mismatch detection
  - Compares CHANGELOG version with BuildConfig
  - Shows warning if versions don't match
  - Still displays content for graceful user experience

### Technical Implementation

- Created `ChangelogLoader.kt` utility (mirrors ExamPackLoader pattern)
  - Uses `context.assets.open("changelog.md")` for file access
  - Returns null on error with proper logging

- Created `ChangelogParser.kt` with markdown parsing logic
  - Extracts first `## [version]` block from changelog
  - Converts markdown to Compose AnnotatedString
  - Regex-based inline formatting (bold, code)
  - Handles nested bullet lists with indentation

- Created `AboutDialog.kt` reusable composable
  - Loads changelog on first composition with `remember(versionName)`
  - Displays version header with build number
  - Shows parsed content with formatting
  - Warns on version mismatch

- Updated `SettingsScreens.kt` and `OriginalSettingsScreen.kt`
  - Replaced hardcoded AlertDialog with AboutDialog composable
  - Both settings screens now use shared component

- Build assets: CHANGELOG.md copied to `app/src/main/assets/changelog.md`
  - Bundled into APK at build time
  - 54KB file size

### Maintenance Workflow

**For future releases:**
1. Update CHANGELOG.md with new version at top
2. Update versionCode/versionName in build.gradle.kts
3. Build app (CHANGELOG auto-copied to assets)
4. AboutDialog automatically shows latest version - no code changes!

### Benefits

- ✅ Zero-maintenance release notes
- ✅ Always shows current version
- ✅ No external dependencies (custom parser vs 500KB library)
...
### UI
- Weekly Study Plan top bar now matches Study Plan Overview header style and strings exactly.
- Daily tab header in Study Plan Overview replaced with compact pill-style back button; removed unrelated full-width bar.
- Bottom navigation converted to a frosted, floating pill with stronger shadow, semi‑transparent gradient, and rounded shape.
- Bottom navigation moved into Scaffold bottomBar to anchor at the bottom and respect system navigation insets; content no longer scrolls underneath.
- Top bars across app adjusted to sit flush at the top (removed extra status-bar padding on Overview and YDS Exam pages).

### Exam Info
- Unified remaining days calculation and safeguarded ÖSYM fetch: only applies fetched sessions when there is at least one upcoming exam; prevents days resetting to 0.

### Build
- No schema changes; project compiles cleanly. Known resource warnings remain about base strings missing for some keys (email, login, etc.).
- Bumped Android versionCode to 90 / versionName 2.9.62.
