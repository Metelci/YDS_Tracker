...
class NotificationSchedulerService @Inject constructor(
    ...
    private fun scheduleDailyStudyReminders() {
        val workRequest = OneTimeWorkRequestBuilder<DailyStudyReminderWorker>()
            .setInputData(
                workDataOf(
                    "notification_manager" to "injected",
                    "app_integration_manager" to "injected"
                )
            )
            .build()

        workManager.enqueueUniqueWork(
            DailyStudyReminderWorker.WORK_NAME,
